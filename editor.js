...
var clip = function(text, from, to) {
...
var getScale = function(elt, rect) {
...
var scrollableParents = function(dom) {
...
var dispatchKey = function(elt, name, code, mods) {
...
var textNodeBefore = function(startNode, startOffset) {
...
var textNodeAfter = function(startNode, startOffset) {
  for (let node = startNode, offset = startOffset;; ) {
...
var autoDirection = function(text, from, to) {
...
var getIsolatedRanges = function(view, line) {
...
var isAtEnd = function(parent, node, offset) {
  for (;; ) {
...
var applyDOMChangeInner = function(view, change, newSel, lastKey = -1) {
...
var bindHandler = function(plugin, handler) {
  return (view, event) => {
    try {
...
var computeHandlers = function(plugins) {
...
var textFilter = function(state2, facet, text) {
...
var clearHeightChangeFlag = function() {
  heightChangeFlag = false;
};
var replace = function(old, val) {
  if (old == val)
    return old;
...
var buildSelectionRangeFromRange = function(view, range) {
...
var safariSelectionRangeHack = function(view, selection) {
...
var wrappedLine = function(view, pos, side, inside) {
...
var isInTooltip = function(tooltip, event) {
...
class EditContextManager {
  ...
  resetRange(state2) {
    ...
    this.to = Math.min(state2.doc.length, head + 1e4);
  }
...
var stackIterator = function(tree, pos, side) {
...
var indentFor = function(stack, cx, pos) {
...
var selectParentSyntax = ({ state: state4, dispatch }) => {
  let selection = updateSel(state4.selection, (range) => {
    var _a2;
    let stack = syntaxTree(state4).resolveStack(range.from, 1);
    for (let cur = stack;cur; cur = cur.next) {
      let { node } = cur;
      if ((node.from < range.from && node.to >= range.to || node.to > range.to && node.from <= range.from) && ((_a2 = node.parent) === null || _a2 === undefined ? undefined : _a2.parent))
...
var deleteByChar = (target, forward, byIndentUnit) => deleteBy(target, (range) => {
  let pos = range.from, { state: state4 } = target, line = state4.doc.lineAt(pos), before, targetPos;
...
var deleteByGroup = (target, forward) => deleteBy(target, (range) => {
  let pos = range.head, { state: state4 } = target, line = state4.doc.lineAt(pos);
  let categorize = state4.charCategorizer(pos);
  for (let cat = null;; ) {
    if (pos == (forward ? line.to : line.from)) {
      if (pos == range.head && line.number != (forward ? state4.doc.lines : 1))
        pos += forward ? 1 : -1;
      break;
    }
...
var createLineDialog = function(view4) {
  let line = String(view4.state.doc.lineAt(view4.state.selection.main.head).number);
  ...
  function go() {
    ...
    if (ln && percent) {
      ...
      line2 = Math.round(state5.doc.lines * pc);
    } else if (ln && sign) {
      line2 = line2 * (sign == "-" ? -1 : 1) + startLine.number;
    }
    let docLine = state5.doc.line(Math.max(1, Math.min(state5.doc.lines, line2)));
...
var defaultPositionInfo = function(view5, list, option, info, space, tooltip) {
...
var sortOptions = function(active, state6) {
  ...
  let conf = state6.facet(completionConfig);
  for (let a of active)
    if (a.hasResult()) {
      ...
      if (a.result.filter === false) {
        ...
        let pattern = state6.sliceDoc(a.from, a.to), match;
...
var getUpdateType = function(tr, conf) {
...
class CompletionContext {
  constructor(state6, pos, explicit, view5) {
    this.state = state6;
    this.pos = pos;
...
class CompletionTooltip {
  constructor(view5, stateField, applyCompletion2) {
    ...
    this.dom.addEventListener("mousedown", (e) => {
      let { options: options2 } = view5.state.field(stateField).open;
      for (let dom = e.target, match;dom && dom != this.dom; dom = dom.parentNode) {
        if (dom.nodeName == "LI" && (match = /-(\d+)$/.exec(dom.id)) && +match[1] < options2.length) {
          this.applyCompletion(view5, options2[+match[1]]);
          e.preventDefault();
          return;
        }
      }
...
var completionPlugin = ViewPlugin.fromClass(class {
  ...
  startQuery(active) {
    ...
    let context = new CompletionContext(state6, pos, active.explicitPos == pos, this.view);
...
var commitCharacters = Prec.highest(EditorView.domEventHandlers({
  keydown(event, view5) {
    let field = view5.state.field(completionState, false);
...
var lintTooltip = function(view6, pos, side) {
  ...
  if (diagnosticFilter)
    found = diagnosticFilter(found, view6.state);
  if (!found.length)
    return null;
  return {
    pos: stackStart,
...
var renderDiagnostic = function(view6, diagnostic, inPanel) {
  ...
  return crelt("li", { class: "cm-diagnostic cm-diagnostic-" + diagnostic.severity }, crelt("span", { class: "cm-diagnosticText" }, diagnostic.renderMessage ? diagnostic.renderMessage(view6) : diagnostic.message), (_a2 = diagnostic.actions) === null || _a2 === undefined ? undefined : _a2.map((action, i) => {
...
class LintState {
  ...
  static init(diagnostics, panel, state7) {
    ...
    if (diagnosticFilter)
      markedDiagnostics = diagnosticFilter(markedDiagnostics, state7);
...
var MMIXAL = (() => {
  ...
  return function(moduleArg = {}) {
    ...
    function findWasmBinary() {
      ...
      return new URL("mmixal.wasm", import.meta.url).href;
    }
    var wasmBinaryFile;
    ...
    var initRandomFill = () => {
      if (typeof crypto == "object" && typeof crypto["getRandomValues"] == "function") {
        return (view7) => crypto.getRandomValues(view7);
      } else
        abort("no cryptographic support found for randomDevice. consider polyfilling it if you want to use something insecure like Math.random(), e.g. put this in a --pre-js: var crypto = { getRandomValues: (array) => { for (var i = 0; i < array.length; i++) array[i] = (Math.random()*256)|0 } };");
    };
    var randomFill = (view7) => {
      return (randomFill = initRandomFill())(view7);
    };
    var PATH_FS = {
      resolve: (...args) => {
    ...
    var FS = {
      ...
      ioctl(stream, cmd2, arg) {
        if (!stream.stream_ops.ioctl) {
          throw new FS.ErrnoError(59);
        }
        return stream.stream_ops.ioctl(stream, cmd2, arg);
      },
      readFile(path, opts = {}) {
    ...
    function ___syscall_fcntl64(fd, cmd2, varargs) {
    ...
    function checkUnflushedContent() {
      ...
      try {
        _fflush(0);
        ["stdout", "stderr"].forEach(function(name2) {
          var info = FS.analyzePath("/dev/" + name2);
          if (!info)
            return;
...
var MMIX = (() => {
  ...
  return function(moduleArg = {}) {
    ...
    ["_malloc", "_memory", "___indirect_function_table", "___asyncjs__read_command_js", "_main", "onRuntimeInitialized"].forEach((prop) => {
      if (!Object.getOwnPropertyDescriptor(readyPromise, prop)) {
        Object.defineProperty(readyPromise, prop, {
          get: () => abort("You are getting " + prop + " on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js"),
          set: () => abort("You are setting " + prop + " on the Promise object, instead of the instance. Use .then() to get called back with the instance, see the MODULARIZE docs in src/settings.js")
    ...
    function findWasmBinary() {
      ...
      return new URL("mmix.wasm", import.meta.url).href;
    }
    var wasmBinaryFile;
    function getBinarySync(file) {
      if (file == wasmBinaryFile && wasmBinary) {
    ...
    function __asyncjs__read_command_js() {
    ...
    var Asyncify = {
      ...
      getDataRewindFuncName(ptr) {
        ...
        var name2 = Asyncify.callStackIdToName[id];
        return name2;
      },
      getDataRewindFunc(name2) {
        var func = wasmExports[name2];
        return func;
      },
      doRewind(ptr) {
        var name2 = Asyncify.getDataRewindFuncName(ptr);
        var func = Asyncify.getDataRewindFunc(name2);
        return func();
      },
...
var mmix_default = MMIX;

// mmix_object_reader.mjs
var read_u32 = function(bytes, i) {
  return BigInt(bytes[i] << 24 | bytes[i + 1] << 16 | bytes[i + 2] << 8 | bytes[i + 3]);
};
var read_u64 = function(bytes, i) {
  let left = read_u32(bytes, i);
  return left << BigInt(32) | read_u32(bytes, i + 4);
};
function parse_mmo(bytes) {
  let cur_line = 0;
  let i = 0;
  let running_spec = -1;
  let quote = false;
...
async function compile() {
  let mmixal2 = await mmixal_default();
  let txt = state9.doc.toString();
  const alfs = mmixal2.FS;
  alfs.writeFile("./code.mms", txt, { encoding: "ascii" });
  mmixal2.callMain(["./code.mms"]);
  let content2 = alfs.readFile("./code.mmo", { encoding: "binary" });
  return content2;
}
var submit_command2 = function() {
  return new Promise((accept, reject) => {
    accept(command2 + "\n");
  });
};
var hello_world_src = `     LOC		Data_Segment
        GREG	@
...
document.getElementById("compile-btn").addEventListener("click", (e) => {
  compile().then((mmo_content) => {
    ...
    mmix_default(MMIX_Module).then((mmix2) => {
      mmix_state.mmix = mmix2;
      mmix_state.loc_to_line = parse_mmo(mmo_content);
      mmix2.FS.writeFile("./code.mmo", mmo_content, { encoding: "binary" });
      mmix2.callMain(["-i", "./code.mmo"]);
    });
  });
});
...
globalThis.submit_command = submit_command2;
