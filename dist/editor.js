      ...
      instrumentFunction(original) {
        var wrapper = (...args) => {
          ...
          try {
            ...
            if (!ABORT) {
              var top2 = Asyncify.exportCallStack.pop();
              assert(top2 === original);
...
function read_u32(bytes, i2) {
  return BigInt(bytes[i2] << 24 | bytes[i2 + 1] << 16 | bytes[i2 + 2] << 8 | bytes[i2 + 3]);
}
function read_u64(bytes, i2) {
  let left = read_u32(bytes, i2);
  return left << BigInt(32) | read_u32(bytes, i2 + 4);
}
  ...
  while (i2 < bytes.length) {
    if (bytes[i2] == 152 && !quote) {
      ...
      switch (bytes[i2 + 1]) {
        ...
        case 1:
          {
            ...
            if (Z == 1) {
              cur_address = Y + read_u32(bytes, i2 + 4);
              i2 += 8;
            } else {
              cur_address = Y + read_u64(bytes, i2 + 4);
        ...
        case 2:
          {
            const YZ = BigInt(bytes[i2 + 2] << 8 | bytes[i2 + 3]);
            cur_address += YZ;
            i2 += 4;
          }
          break;
        ...
        case 7:
          {
            const YZ = bytes[i2 + 2] << 8 | bytes[i2 + 3];
            cur_line = YZ;
            i2 += 4;
          }
          break;
...
Main	LDA		$255,Text
        ...
        TRAP	0,Halt,0
`;
var mmix_tokenizer = StreamLanguage.define({
  startState() {
    return {};
  },
  token(stream) {
    if (stream.eatSpace())
      return null;
    if (stream.sol()) {
      if (stream.match(/^[A-Za-z_][_A-Za-z0-9]*/))
        return "tag";
    }
    if (stream.match(/^;[A-Za-z_][_A-Za-z0-9]*/))
      return "tag";
    ...
    if (stream.match(/\b(BYTE|WYDE|TETRA|OCTA)\b/))
      return "keyword";
    if (stream.match(/\b(LOC|GREG|IS|BYTE|WYDE|TETRA|OCTA)\b/))
      return "keyword";
    if (stream.match(/^\b([A-Za-z]+[0-9]*|\d+H)\b\s/))
      return "definition";
    if (stream.match(/^(\/\/|;|%|#\s).*$/))
      return "comment";
    if (stream.match(/^\$\d{1,3}\b/))
      return "constant";
    ...
    if (stream.match(/^-?[0-9]+\b/))
      return "number";
    if (stream.match(/^(#|0x)[0-9a-fA-F_]+\b/))
      return "number";
    if (stream.match(/^@/))
      return "constant";
...
var setHighlightLine = StateEffect.define();
...
var breakpointState = StateField.define({
  create() {
    return RangeSet.empty;
  },
...
var mmix_state = {
  ...
  init: async function(mmo_content) {
    ...
    const mmix = await mmix_default(MMIX_Module);
    this.mmix = mmix;
...
`;
  ...
  send_command_with_output: async function(cmd2) {
    await this.pause_until_ready();
...
`)) {
      ...
      for (let i2 = 0;i2 < 16; i2 += 2) {
        result.push(parseInt(`0x${octa_padded.slice(i2, i2 + 2)}`));
      }
    }
    return result;
  },
  update_register_table: async function(change_visibility = true) {
    ...
    for (let i2 = 0;i2 < rL; i2++)
      await update(this, `regr${i2}`, `$${i2}`);
    for (let i2 = rG;i2 <= 255; i2++)
      await update(this, `regr${i2}`, `$${i2}`);
    for (let i2 = 65;i2 <= 90; i2++) {
      const reg = String.fromCharCode(i2);
  ...
  update_memory_table: async function() {
    ...
    for (let i2 = 0;i2 < bytes.length; i2 += 8) {
      const row = document.createElement("tr");
      const address = BigInt(`0x${start_address}`) + BigInt(i2);
      ...
      var cell_content = "";
      for (let j = i2;j < i2 + 8; j += bytes_per_element) {
        let value = BigInt(0);
...
async function compile() {
  let mmixal = await mmixal_default();
  let txt = editor.state.doc.toString();
  let maxLength = 80;
  for (let i2 = 0;i2 < editor.state.doc.lines; i2++) {
    const line = editor.state.doc.line(i2 + 1);
...
var stdout_area = document.getElementById("the-terminal");
...
async function step_or_coninue(command2) {
...
var show_github_load_btn = document.getElementById("show-load-github-btn");
var github_load_btn = document.getElementById("load-github-btn");
...
var editorElement = document.getElementById("editor-div");
resizeObserver.observe(editorElement);
var register_format_select = document.getElementById("register-format");
register_format_select.addEventListener("change", async (e) => {
  await mmix_state.update_register_table(false);
});
var add_register_btn = document.getElementById("add-register-btn");
...
var load_memory_btn = document.getElementById("load-memory-btn");
var memory_format_select = document.getElementById("memory-format");
var memory_bytes_select = document.getElementById("memory-bytes");
...
var memory_address_cache = document.getElementById("memory-address-cache");
